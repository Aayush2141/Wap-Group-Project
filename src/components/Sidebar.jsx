...
function NavItem({ to, icon: Icon, label, exact = false }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors duration-200 ${isActive ? 'text-white' : 'text-[#a7a7a7] hover:text-white'}`
      }
    >
      {({ isActive }) => (
        <>
          {isActive && <div className="absolute inset-0 bg-white/8 rounded-xl" />}
          <Icon size={20} className="relative z-10 flex-shrink-0" fill={isActive ? 'currentColor' : 'none'} strokeWidth={isActive ? 2.5 : 2} />
          <span className="relative z-10">{label}</span>
          {isActive && <div className="relative z-10 ml-auto w-1.5 h-1.5 rounded-full bg-[#1db954]" />}
        </>
      )}
    </NavLink>
  );
...
function RecentItem({ song }) {
  ...
  return (
    <li
      ...
      className={`flex items-center gap-3 px-2 py-2 rounded-xl cursor-pointer transition-colors duration-150 group ${active ? 'bg-white/8' : 'hover:bg-white/5'}`}
    >
      <div className="relative w-10 h-10 flex-shrink-0">
        <img src={song.album?.coverSmall || song.album?.cover} alt={song.title} className="w-10 h-10 rounded-lg object-cover" />
        {active && isPlaying ? (
      ...
      <div className="overflow-hidden">
        <p className={`text-xs font-semibold truncate leading-tight ${active ? 'text-[#1db954]' : 'text-white'}`}>{song.title}</p>
  ...
  return (
    <aside className="w-[240px] flex-shrink-0 flex flex-col gap-2 p-2 overflow-hidden bg-[#050505]">
      ...
      <div className="bg-[#111111] rounded-2xl p-3 flex-1 overflow-y-auto">
        <div className="flex items-center gap-2 mb-3 px-1">
          ...
          {recentlyPlayed.length > 0 && (
            <span className="ml-auto text-[10px] text-[#535353] bg-white/5 rounded-full px-1.5 py-0.5">{recentlyPlayed.length}</span>
          )}
        </div>

        {recentlyPlayed.length === 0 ? (
          <p className="text-[#535353] text-xs px-1 py-4 text-center leading-relaxed">Songs you play<br />will appear here</p>
        ) : (
      ...
      {likedSongs.length > 0 && (
        <NavLink
          to="/liked"
          className="mx-0.5 mb-1 flex items-center gap-3 rounded-2xl p-3 overflow-hidden relative bg-gradient-to-r from-indigo-900/60 to-purple-900/60 border border-white/5 hover:border-white/10 transition-all group"
        >
