// WHAT THIS FILE DOES:
// The Artist page — fetches and displays an artist's profile and their top tracks.
// The artist ID comes from the URL (e.g. /artist/159260351) via useParams.
...
// A single row in the artist's track list
function TrackRow({ song, index, queue }) {
  // We use context here to access player controls
  ...
  const handlePlay = () => {
    if (active) {
      ...
      playSong(song, queue); // play this song with the full artist track list as queue
    }
  };
  ...
  return (
    ...
    >
      {/* Row number or EQ bars when playing */}
      ...
      <button
        ...
        className={`p-1 transition-all opacity-0 group-hover:opacity-100 active:scale-90
...
// The main Artist Page component
export default function ArtistPage() {
  // useParams reads the :artistId segment from the URL
  const { artistId } = useParams();

  // useFetchArtist fetches both artist info and their top tracks
  const { artist, songs, loading, error } = useFetchArtist(artistId);

  // We use context here to access playSong, currentSong, isPlaying, togglePlay
  const { playSong, currentSong, isPlaying, togglePlay } = usePlayer();

  const navigate = useNavigate();

  // True when any of this artist's songs is currently playing
  const isArtistActive = songs.some(s => s.id === currentSong?.id) && isPlaying;

  const handlePlayAll = () => {
    if (!songs.length) return;
    if (isArtistActive) {
      togglePlay(); // pause if already playing this artist
    } else {
      playSong(songs[0], songs); // start from the first track
    }
  };
  ...
  // Generate a consistent gradient hue from the artist's name characters
  ...
  return (
    <div className="page-enter">
      ...
      <div className="px-8 pt-6">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-1.5 text-[#a7a7a7] hover:text-white text-sm transition-colors
            bg-black/40 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/5 w-fit"
        >
      ...
      {loading ? (
        ...
        <div
          className="relative px-8 pt-6 pb-8 flex items-end gap-6 mt-4 rounded-2xl mx-6"
          style={{ background: gradientBg }}
        >
          {/* Artist avatar — gradient circle showing the first letter of the artist's name */}
          <div
            className="w-36 h-36 rounded-full flex-shrink-0 flex items-center justify-center
              border-4 border-white/10 shadow-2xl text-5xl font-black text-white select-none"
          ...
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white leading-none mb-2 drop-shadow-lg">
              {artistName}
            </h1>
      ...
      <div className="px-8 py-5 flex items-center gap-4">
        <button
          ...
          className="w-14 h-14 rounded-full bg-[#1db954] hover:bg-[#1ed760] flex items-center justify-center
            shadow-lg shadow-green-900/40 disabled:opacity-40 transition-all
            hover:scale-106 active:scale-94"
        >
          {isArtistActive
