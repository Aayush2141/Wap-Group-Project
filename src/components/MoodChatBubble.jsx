  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-7 h-7 rounded-full bg-[#1db954] flex items-center justify-center flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all hover:bg-[#1ed760] hover:scale-110"
...
export default function MoodChatBubble() {
  ...
  const processMessage = async (text) => {
    ...
    setMessages(prev => [...prev, {
      id: Date.now() + 1, role: 'bot',
      text: songs.length > 0 ? `Here are some ${detected.mood} vibes 🎧` : `I detected a ${detected.mood} mood but couldn't reach the music service. Try again in a sec!`,
      ...
      >
        ...
        <div className="flex gap-1.5 px-4 py-2.5 flex-shrink-0 overflow-x-auto" style={{ scrollbarWidth: 'none', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          {CHIPS.map(chip => (
            <button key={chip} onClick={() => handleChip(chip)} disabled={isLoading}
              className="flex-shrink-0 text-[10px] font-semibold px-2.5 py-1 rounded-full text-[#a7a7a7] hover:text-white transition-all disabled:opacity-40 hover:scale-105 active:scale-95"
        ...
        <div className="flex items-center gap-2.5 px-3.5 py-3 flex-shrink-0" style={{ borderTop: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.3)' }}>
          ...
          <button
            ...
            className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:scale-110 active:scale-90"
            style={{
      ...
      <div className="fixed right-5 z-50 flex items-center gap-2.5 group" style={{ bottom: '108px' }} onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)}>
        ...
        <button
          ...
          style={{
            ...
            boxShadow: isOpen ? '0 4px 16px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.1)' : '0 4px 24px rgba(29,185,84,0.4), 0 0 0 1px rgba(29,185,84,0.2)',
            border: 'none',
        ...
        >
          ...
          {!isOpen && (
            <span className="absolute top-0.5 right-0.5 w-2.5 h-2.5 rounded-full bg-[#1db954] border-2" style={{ borderColor: '#0a0a0a', animation: 'pulse-dot 2.5s ease-in-out infinite' }} />
          )}
        </button>
      </div>
    </>
